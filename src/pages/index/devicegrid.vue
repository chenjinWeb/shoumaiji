                ...
                <a class="central_list flex">
                  <div class="list_lef list_left storescont">
                    ...
                    <p class="time">
                      编号：{{l.number}}
                      <span>
                        <img src="../../assets/img/guizi.png"/>
                      </span>
                    </p>
                  </div>
                  <div class="list_rig list_right clear">
                    <p class="fl line_state">
                        <span class="empty_path" @click.stop="check(l.id,l.offaisle)" style="margin-right: 0.17rem;">
                          <em>{{l.offaisle}}</em>
                          空道
                        </span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      childpage(id, type) {
        this.$router.push({name: "griddetail", query: {id: id, type: type}})
      },
      goPage(name) {
      ...
      check(id, num) {
        if (num > 0) {
          ...
          this.arr = [];
          this.offMonitorInfo_({vmId: id,grid:1}).then((res) => {
